...
export class CrqsasService {
  ...
  constructor(
   ...
   createCrqsas(crqsas: Crqsas, patient_id: number){
    ...
    return this.http.post('http://localhost:8000/api/crq_sas?token=' + token, {crqsas, patient_id}, {headers: headers})
    .toPromise();
  }
  ...
  updateCrqsas(crqsas: Crqsas, patient_id: number){
    ...
    const token = this._authService.getToken();
    const headers = new HttpHeaders({'Content-Type': 'application/json'})
    return this.http.put(`http://localhost:8000/api/crq_sas/${crqsas.id}?token=` + token, {crqsas, patient_id}, {headers: headers})
    .toPromise();
  }
  ...
  getCrqsas(id:number){
    ...
    const token = this._authService.getToken();
    const headers = new HttpHeaders({'Content-Type': 'application/json'})
    return this.http.get<Crqsas>(`http://localhost:8000/api/crq_sas/${id}?token=` + token);
  }
